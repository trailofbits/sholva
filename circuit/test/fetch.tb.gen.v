...
always @(negedge clk)
  ...
  $display("TBGEN-FAIL fetch:%0d expected raw_instr to be %h, was %h", current_vec, exp_raw_instr, raw_instr);
...
if (exp_raw_regs !== raw_regs) begin
  $display("TBGEN-FAIL fetch:%0d expected raw_regs to be %h, was %h", current_vec, exp_raw_regs, raw_regs);
  error_count = error_count + 1;
end else begin
...
if (exp_raw_hint1 !== raw_hint1) begin
  $display("TBGEN-FAIL fetch:%0d expected raw_hint1 to be %h, was %h", current_vec, exp_raw_hint1, raw_hint1);
  error_count = error_count + 1;
end else begin
...
if (exp_raw_hint2 !== raw_hint2) begin
  $display("TBGEN-FAIL fetch:%0d expected raw_hint2 to be %h, was %h", current_vec, exp_raw_hint2, raw_hint2);
  error_count = error_count + 1;
end else begin
