...
impl From<&clap::ArgMatches> for Tracer {
    fn from(matches: &clap::ArgMatches) -> Self {
        let target = if let Some(pid) = matches.get_one::<String>("tracee-pid") {
            ...
            Target::Program(
                matches.get_one::<String>("tracee-name").map(Into::into).unwrap(),
                matches
                    .get_many::<String>("tracee-args")
        ...
        #[allow(clippy::redundant_field_names)]
        Self {
            ignore_unsupported_memops: matches.contains_id("ignore-unsupported-memops"),
            tiny86_only: matches.contains_id("tiny86-only"),
            decree_syscalls: matches.get_one::<String>("syscall-model").unwrap() == "decree",
            debug_on_fault: matches.contains_id("debug-on-fault"),
            disable_aslr: matches.contains_id("disable-aslr"),
